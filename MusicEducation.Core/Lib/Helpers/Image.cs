...
namespace MusicEducation.Core.Lib.Helpers
{
	abstract public class BaseFile
  {
	...
	public class ImageFormated
	{
		...
		public ImageFormated(string imageRelativePath, string imageName)
		{
			if (String.IsNullOrEmpty(imageName))
			{
				imageRelativePath = "Uploads\\Images";
				imageName = "ron_240x272.png";
			}
		...
		public ImageFormated(string imageFromUploadFullName) : this(UploadRelativeDir, imageFromUploadFullName) { }
		...
		public static string SaveImage(Image image, string format, HttpServerUtility server)
		{
			string fileName = String.Format("{0}.{1}", Guid.NewGuid().ToString(), format);
		...
		public class Validation
		{
			...
			public void ValidateImage()
			{
				...
				if (!isValieFormat)
					throw new Exception("Неправильный формат изображения");

				if (isValideSizeof)
					throw new Exception("Рамер фото больше допустимого");
			}
		}
	}
}